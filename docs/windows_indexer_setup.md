...
- В `file-index` отправляется весь снимок файлов за один раз (`Files[]`), поэтому на больших папках лимит легко превысить.
- Проверьте:
  - что `scanRoot` не указывает на слишком большую директорию (лучше сузить до рабочей папки);
  - что на сервере и reverse-proxy увеличен лимит тела запроса для `/api/file-index/sync`.
